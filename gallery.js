    ...
    function renderBatch(startIndex) {
      ...
      for(let i = startIndex; i < endIndex; i++) {
        ...
        if(window.__VC_MODE && window.__REMOTE_PORTFOLIO_BASE){
          const idTok = window.__VC_ID_TOKEN || (function(){ try { return localStorage.getItem('vcIdToken'); } catch(_){ return null; }})();
          if(idTok){
            a.href = `portfolio.html?driveFile=${encodeURIComponent(m.file)}&t=${timestamp}&idToken=${encodeURIComponent(idTok)}`;
          } else {
            a.href = `portfolio.html?driveFile=${encodeURIComponent(m.file)}&t=${timestamp}`;
          }
        } else {
  ...
  async function loadIndex(){
    ...
    if(window.__VC_MODE && window.__REMOTE_MANIFEST_URL){
      try {
        ...
        metaList = arr.map(m => ({ file: m.file || m.roll || 'unknown.json', name: (m.roll || m.file || 'untitled').toString() }));
