...
async function saveToTemporaryStorage(drive, filename, portfolio, email, metadata) {
  try {
    // Ensure temporary folder exists under the shared parent folder to leverage its quota
    // (Service accounts have no personal My Drive quota)
    let tempFolderId = await getOrCreateFolder(drive, TEMP_FOLDER_NAME, DRIVE_PARENT_FOLDER_ID);
...
async function emptyServiceAccountTrash(drive) {
  try {
    const finalFolderId = await getOrCreateFolder(drive, FINAL_FOLDER_NAME, DRIVE_PARENT_FOLDER_ID);
    const tempFolderId = await getOrCreateFolder(drive, TEMP_FOLDER_NAME, DRIVE_PARENT_FOLDER_ID);
...
app.get('/api/list', async (req,res)=>{
  try{
    ...
    if (!token) {
      console.log('[List] No token provided');
      return res.status(401).json({ error: 'Authentication required' });
    }
    
    console.log('[List] Verifying token for admin access...');
    const { email } = await verifyToken(token);
    
    console.log(`[List] Token verified for email: ${email}`);
    if (email !== ADMIN_EMAIL) {
      console.log(`[List] Access denied - not admin email: ${email} vs ${ADMIN_EMAIL}`);
    ...
    // Additionally, include any JSON files that exist directly under the configured parent folder
    // (older uploads or externally populated files). These are treated as root-level legacy items.
    try {
      const rootLegacyResp = await drive.files.list({
      ...
      (rootLegacyResp.data.files || []).forEach(f => {
        const props = f.appProperties || {};
        items.push({
          id: f.id,
          file: f.name.replace(/\.json$/, ''),
          name: f.name,
          roll: props.roll || extractRollFromFilename(f.name) || '',
          email: props.email || props.userId || '',
          updatedAt: f.modifiedTime,
          uploadedBy: props.uploadedBy || props.userId || '',
          createdAt: props.createdAt || f.modifiedTime,
          version: props.version || 'legacy-root',
          size: parseInt(f.size || '0'),
          userFolder: 'parent-root'
        });
      });

      console.log(`[List] Found ${rootLegacyResp.data.files?.length || 0} root-level legacy files under parent`);
    } catch (err) {
      console.warn('[List] Error scanning parent-root legacy files:', err.message);
    }
